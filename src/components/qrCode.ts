...
class QRRSBlock {
    ...
    getRSBlocks(typeNumber: number, errorCorrectionLevel: QRErrorCorrectionLevel) {
...
var QRUtil = function() {
	...
	var getBCHDigit = function(data: number) {
		var digit = 0;
		while (data != 0) {
			digit += 1;
			data >>>= 1;
	...
	_this.getLengthInBits = function(mode: QREncoding, type) {
	...
	_this.getLostPoint = function(moduleCount, isDarkFunc) {
