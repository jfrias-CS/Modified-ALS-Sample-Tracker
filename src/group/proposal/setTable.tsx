...
import {
    LoadingBanner,
    LoadingState,
} from "../../components/loadingBanner.tsx";
...
const SetTable: React.FC = () => {
    ...
    useEffect(() => {
        if (metadataContext.setsLoadingState == MetaDataLoadingState.Failed) {
            ...
            setLoadingMessage(
                "Failed to load Sets. Are you sure you're still logged in?"
            );
        } else if (
            metadataContext.loadingState == MetaDataLoadingState.Succeeded
        ) {
            const sortedSets = metadataContext.sets.allValid().sort((a, b) => {
    ...
    function refreshSets() {
        const sortedSets = metadataContext.sets
            .allValid()
            .sort((a, b) => sortWithNumberParsing(a.name, b.name));
        setSets(sortedSets);
    }
    ...
    return (
        <>
            <nav className="breadcrumb is-medium" aria-label="breadcrumbs">
                <ul>
                    <li>
                        <Link to={"/group/" + groupContext.group!.id}>
                            Proposals
                        </Link>
                    </li>
            ...
            <nav className="level">
                <div className="level-left">
                    <div className="level-item">
                        <p className="subtitle is-5">
                            <strong>{sets.length}</strong> bars
                        </p>
                    </div>
                </div>
                ...
                <div className="level-right">
                    ...
                    <div className="level-item">
                        <p className="subtitle is-5">
                            <Link to={"labels"}>Printable Labels</Link>
                        </p>
                    </div>
                </div>
            </nav>
