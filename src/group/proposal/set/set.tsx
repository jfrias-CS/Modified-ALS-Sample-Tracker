...
import {
    InputEditable,
    ValidationStatus,
} from "../../../components/inputEditable.tsx";
...
const Set: React.FC = () => {
    ...
    useEffect(() => {
        appConfig.log(
            `set setId:${setId} changeCounter:${metadataContext.changeCounter} setsLoadingState:${metadataContext.setsLoadingState} scanTypesLoadingState:${metadataContext.scanTypesLoadingState}`
        );
    ...
    async function nameEditValidate(value: string) {
        ...
        if (trimmed.length < 1) {
            ...
            metadataContext.sets.allValid().some((c) => c.name == trimmed)
        ) {
            return {
                status: ValidationStatus.Failure,
                message: "Name must be unique for proposal.",
            };
        }
                    ...
                    <div className="level-item">
                        ...
                        >
                            <ul>
                                <li>
                                    <Link
                                        to={"/group/" + groupContext.group!.id}
                                    >
                                        Proposals
                                    </Link>
                                </li>
                        ...
                        <div className="dropdown is-right is-hoverable">
                            <div className="dropdown-trigger">
                                <button
                                    className="button"
                                    aria-haspopup="true"
                                    aria-controls="dropdown-menu-howto"
                                >
