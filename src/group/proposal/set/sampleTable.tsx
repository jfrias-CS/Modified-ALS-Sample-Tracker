...
import {
    ...
    SampleConfigurationField,
    SampleConfigurationFieldSelection,
} from "../../../sampleConfiguration.ts";
import {
    MetadataContext,
    MetaDataLoadingState,
} from "../../../metadataProvider.tsx";
...
} from "./sampleTableCell/cellDto.ts";
...
function findAnEditableCell(node: HTMLElement): Coordinates | null {
    ...
    if (!p) {
        ...
        return findAnEditableCell(p as HTMLElement);
    }
}
...
function findEnclosingTable(node: HTMLElement): HTMLElement | null {
    ...
    if (!p) {
        ...
        return findEnclosingTable(p as HTMLElement);
    }
}
...
const SampleTable: React.FC<SampleTableProps> = (props) => {
    ...
    const [lastActivationMethod, setLastActivationMethod] =
        useState<CellActivationStatus>(CellActivationStatus.Inactive);
    const tableRef = useRef<HTMLTableElement>(null);

    const [tableSortColumn, setTableSortColumn] = useState<SampleConfigurationFieldSelection>(
        { field: SampleConfigurationField.Name, parameter: null, }
    );
    const [tableSortReverse, setTableSortReverse] = useState<boolean>(false);

    const [cellMouseDown, setCellMouseDown] = useState<boolean>(false);         // A cell has been clicked
    const [cellMouseDownX, setCellMouseDownX] = useState<number | null>(null);  // X coordinate of the clicked cell
    ...
    const [selectedSampleIds, setSelectedSampleIds] = useState<Set<Guid>>(new Set());
    ...
    }, [setId, metadataContext.changeCounter, metadataContext.loadingState, metadataContext.sets, tableSortColumn, tableSortReverse]);
    ...
    function sortSampleIds(
        ...
        field: SampleConfigurationFieldSelection,
        reverse: boolean
    ): Guid[] {
        ...
        switch (field.field) {
            ...
            default: // Default is to sort by Name
                sorted = ids.sort((a, b) => {
                    return sortWithNumberParsing(a.name, b.name, reverse);
                });
                break;
            case SampleConfigurationField.Parameter:
                if (field.parameter !== null) {
                    sorted = ids.sort((a, b) => {
                        return sortWithNumberParsing(
                            a.parameters.get(field.parameter!),
                            b.parameters.get(field.parameter!),
                            reverse
                        );
                    });
                }
    ...
    function clickedTableSortHeader(f: SampleConfigurationFieldSelection) {
        ...
        if (
            f.field == tableSortColumn.field &&
            f.parameter == tableSortColumn.parameter
        ) {
            const sortedSampleIds = sortSampleIds(
                thisSet,
                tableSortColumn,
    ...
    function tableOnPointerDown(event: React.PointerEvent<HTMLElement>) {
        ...
        // if (foundCell) {
            ...
            console.log("tableOnPointerDown first selected SampleID:", sortedSampleIds[foundCell.y]);
    ...
    function tableOnPointerOver(event: React.PointerEvent<HTMLElement>) {
        ...
        /*
        // Update the selection when dragging 
        // if (cellMouseDownY !== null && foundCell.y !== cellMouseMoveY) {
            ...
            if (cellMouseDownX !== null && cellMouseDownY !== null) {
                ...
                if (travelVector !== null) {
                    if (
                        ...
                        const found = lookForAdjacentCell(
                            { x: cellMouseMoveX, y: cellMouseMoveY },
                            travelVector
                        );
                        if (found !== null) {
                        ...
                        if (
                            ...
                            found = lookForAdjacentCell(
                                { x: cellMouseMoveX, y: cellMouseMoveY },
                                travelVector
                            );
                        } else {
                            found = lookForAdjacentCell(
                                { x: cellMouseDownX, y: cellMouseDownY },
                                travelVector
                            );
                        }
                        if (found !== null) {
    ...
    function isWithinSelection(x: number, y: number) {
        if (
            ...
            if (
                x <= Math.max(cellMouseDownX, cellMouseMoveX) &&
                x >= Math.min(cellMouseDownX, cellMouseMoveX)
            ) {
                if (
                    y <= Math.max(cellMouseDownY, cellMouseMoveY) &&
    ...
    const displayedParameters = displayedParameterIds.map(
        (p) => metadataContext.scanTypes.parametersById.get(p)!
    );
    ...
    function tableOnCopy(event: React.ClipboardEvent) {
        ...
        if (cellFocusY !== null || cellFocusX !== null) {
            return;
        }
        // No rows selected
        if (cellMouseDownY === null || cellMouseMoveY === null) {
            return;
        }
        // No columns selected
        if (cellMouseDownX === null || cellMouseMoveX === null) {
            return;
        }
        const c: SampleTableClipboardContent =
            new SampleTableClipboardContent();
        ...
        for (var y = lowerY; y <= higherY; y++) {
            sampleRows.push(
                thisSet!.configurationsById.get(sortedSampleIds[y])!
            );
        }

        // The first FIXED_COLUMN_COUNT columns of the table always represent these fields in order:
        const fields = [
            SampleConfigurationField.Name,
            SampleConfigurationField.Description,
            SampleConfigurationField.ScanType,
        ];
        var selectedFields = [];
        ...
        while (lowX <= higherX) {
            selectedParameters.push(
                displayedParameterIds[lowX - FIXED_COLUMN_COUNT]
            );
            lowX++;
        }
    ...
    function tableOnPaste(event: React.ClipboardEvent) {
        ...
        const clipboardColumnCount =
            c.selectedFields.length + c.selectedParameters.length;
        ...
        if (lowerRightX > upperLeftX) {
            ...
            if (clipboardColumnCount > 1) {
                stopAtColumn = Math.min(
                    stopAtColumn,
                    upperLeftX + clipboardColumnCount
                );
            }
        }
        ...
        stopAtColumn = Math.min(
            stopAtColumn,
            FIXED_COLUMN_COUNT + displayedParameters.length
        );
        ...
        while (upperLeftY + y < stopAtRow) {
            ...
            const allowedParameters = new Set(
                thisScanType.parameters.map((p) => p.typeId)
            );
            ...
            const scanParameterSettingsMap: Map<
                ParamUid,
                ScanParameterSettings
            > = new Map(thisScanType.parameters.map((sp) => [sp.typeId, sp]));

            const currentClipboardRow =
                pasteValues.length == 1 ? pasteValues[0] : pasteValues[y];

            var x = 0;
            while (upperLeftX + x < stopAtColumn) {
                var pasteValue =
                    currentClipboardRow.length == 1
                        ? currentClipboardRow[0]
                        : currentClipboardRow[x];
                if (pasteValue === null) {
                    pasteValue = "";
                }

                // Name
                if (upperLeftX + x === 0 && pasteValue !== "") {
                    // Don't allow blank names
                    editedConfig.name = pasteValue.replace(
                        /[^A-Za-z0-9\-_]/g,
                        "_"
                    );
                    ...
                    if (newScanType) {
                        ...
                        newScanType!.parameters.forEach((p) => {
                            const parameterType =
                                metadataContext.scanTypes.parametersById.get(
                                    p.typeId
                                );
                            if (parameterType) {
                                // Set any missing parameters to defaults.
                                if (
                                    !editedConfig.parameters.has(
                                        parameterType!.id
                                    )
                                ) {
                                    editedConfig.parameters.set(
                                        parameterType.id,
                    ...
                    upperLeftX + x >= FIXED_COLUMN_COUNT &&
                    upperLeftX + x - FIXED_COLUMN_COUNT <
                    ...
                    const scanParameterSettings:
                        | ScanParameterSettings
                        | undefined = scanParameterSettingsMap.get(
                        paramType.id
                    );
    ...
    function lookForAdjacentCell(
        ...
        const thisParameter =
            displayedParameters[nextPosition.x - FIXED_COLUMN_COUNT];
        const thisConfig = thisSet!.configurationsById.get(
            sortedSampleIds[nextPosition.y]
        );

        const allowedParameters = metadataContext.scanTypes.typesByName.get(
            thisConfig!.scanType
        )!.parameters;
    ...
    function switchEditingToNearbyCell(
        currentPosition: Coordinates,
        vector: Coordinates
    ): CellValidationStatus {
    ...
    function cellValidator(
        ...
        if (x === 0) {
            if (value == "") {
                return {
                    status: CellValidationStatus.Failure,
                ...
                if (configsMatchingName.length > 1) {
                    return {
                        status: CellValidationStatus.Failure,
                        message: "Name must be unique on bar.",
                    };
                } else if (value.search(/[^A-Za-z0-9\-_]/g) >= 0) {
                    return {
                        status: CellValidationStatus.Failure,
                        message:
                            "Name must consist only of letters, numbers, underscore, or dash.",
                    };
                }
            }
            ...
            if (
                metadataContext.scanTypes.typeNamesInDisplayOrder.some(
                    (name) => name == value
                )
            ) {
                return { status: CellValidationStatus.Success, message: null };
            ...
            return {
                status: CellValidationStatus.Failure,
                message: "Must be the name of a Scan Type.",
            };

            // Validate scan parameters
        } else if (
            x >= FIXED_COLUMN_COUNT &&
            x - FIXED_COLUMN_COUNT < displayedParameters.length
        ) {
        ...
        return {
            status: CellValidationStatus.Failure,
            message: "Error: Cannot find this parameter!",
        };
    }
    ...
    function cellSave(
        ...
        if (!thisConfig) {
            return {
                status: CellValidationStatus.Failure,
                message: "Error: SampleConfiguration does not exist!",
            };
        }
        ...
        if (x === 0) {
            ...
            const newScanType =
                metadataContext.scanTypes.typesByName.get(asScanTypeName);
            // If the text doesn't resolve to a known Scan Type name, skip all this.
            // We accept invalid values in most columns but accepting one in ScanType would make very ambiguous behavior,
            // since it affects which columns are displayed in the table.
            if (newScanType) {
                ...
                newScanType!.parameters.forEach((p) => {
                    const parameterType =
                        metadataContext.scanTypes.parametersById.get(p.typeId);
                    if (parameterType) {
                        // Set any missing parameters to defaults.
    ...
    const tableHeaders = [
        <th
            key="name"
            className={truthyJoin(
                "sortable",
                tableSortColumn.field == SampleConfigurationField.Name &&
                    sortDirectionStyle
            )}
            onClick={() =>
                clickedTableSortHeader({
                    field: SampleConfigurationField.Name,
                    parameter: null,
                })
            }
            scope="col"
        ...
        <th
            ...
            className={truthyJoin(
                "sortable",
                tableSortColumn.field == SampleConfigurationField.Description &&
                    sortDirectionStyle
            )}
            onClick={() =>
                clickedTableSortHeader({
                    field: SampleConfigurationField.Description,
                    parameter: null,
                })
            }
            scope="col"
        ...
        <th
            ...
            className={truthyJoin(
                "sortable",
                tableSortColumn.field == SampleConfigurationField.ScanType &&
                    sortDirectionStyle
            )}
            onClick={() =>
                clickedTableSortHeader({
                    field: SampleConfigurationField.ScanType,
                    parameter: null,
                })
            }
            scope="col"
            ...
            <th
                ...
                className={truthyJoin(
                    ...
                    tableSortColumn.field ==
                        SampleConfigurationField.Parameter &&
                        tableSortColumn.parameter == p.id &&
                        sortDirectionStyle
                )}
                onClick={() =>
                    clickedTableSortHeader({
                        field: SampleConfigurationField.Parameter,
                        parameter: p.id,
                    })
                }
                scope="col"
    ...
    let syncStatusMessage: JSX.Element | null = null;
    if (syncState == SyncState.Requested) {
        syncStatusMessage = (
            <div>
                <FontAwesomeIcon icon={faSpinner} spin={true} /> Saving Changes
            </div>
        );
        ...
        syncStatusMessage = (
            <div>
                <FontAwesomeIcon icon={faExclamationTriangle} color="darkred" />{" "}
                Error Saving! Are you logged in?
            </div>
        );
    }
                ...
                {sortedSampleIds.length == 0 ? (
                    !tableHelpDisclosed ? (
                        <nav className="level">
                            <div className="level-left">
                                <div className="level-item">
                                    <p>
                                        (You might want to check out this brief
                                    </p>
                                </div>
                                <div className="level-item">
                                    <button
                                        className="button is-success"
                                        onClick={onTableHelpShow}
                                    >
                                        Tutorial
                                    </button>
                                </div>
                            ...
                            <div className="modal-card">
                                ...
                                <section className="modal-card-body">
                                    <video
                                        ...
                                        }/sample_table_video_tutorial.mp4`}
                                        controls={true}
                                        autoPlay={true}
                                        loop={true}
                                        muted={true}
                    ...
                    >
                        ...
                        <tbody>
                            {sortedSampleIds
                                .filter((sId) =>
                                    thisSet!.configurationsById.get(sId)
                                )
                                .map((sampleId, sampleIndex) => {
                                    const sample =
                                        thisSet!.configurationsById.get(
                                            sampleId
                                        )!;
                                    ...
                                    const cellFocusOnThisY =
                                        cellFocusY === sampleIndex;
                                    const allowedParameters = new Set(
                                        thisScanType.parameters.map(
                                            (p) => p.typeId
                                        )
                                    );
                                    // Build a temporary map, from parameter Ids to their ScanParameterSettings in this row's (config's) scan type.
                                    ...
                                    const scanParameterSettingsMap: Map<
                                        ...
                                        ScanParameterSettings
                                    > = new Map(
                                        thisScanType.parameters.map((sp) => [
                                            sp.typeId,
                                            sp,
                                        ])
                                    );

                                    displayedParameters.forEach(
                                        (p, paramIndex) => {
                                            const scanParameterSettings:
                                                | ScanParameterSettings
                                                | undefined =
                                                scanParameterSettingsMap.get(
                                                    p.id
                                                );

                                            const unused =
                                                !allowedParameters.has(p.id);
                                            // readOnly may not be defined, so this could effectively still be true | false | undefined.
                                            const readOnly =
                                                scanParameterSettings?.readOnly;
                                            const activated =
                                                cellFocusX ===
                                                    paramIndex +
                                                        FIXED_COLUMN_COUNT &&
                                            ...
                                            const activationStatus = activated
                                                ...
                                                : CellActivationStatus.Inactive;
                                            const paramValue =
                                                sample.parameters.get(p.id) ??
                                                "";
                                    ...
                                    const nameValidationResult = cellValidator(
                                        0,
                                        sampleIndex,
                                        sample.name
                                    );
                                    const nameValidValue =
                                        nameValidationResult.status ==
                                        CellValidationStatus.Success
                                            ? true
                                            : false;
                                    ...
                                    const descriptionValidValue =
                                        descriptionValidationResult.status ==
                                        CellValidationStatus.Success
                                            ? true
                                            : false;
                                    ...
                                    return (
                                        <tr
                                            ...
                                            className={selectedSampleIds.has(sampleId) ? "is-shift-selected" : ""}
